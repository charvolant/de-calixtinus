...
COPY . /build/
RUN mkdir -p /app/lib && mkdir -p /app/bin && cp -r static config-static.yaml camino-portuguese.json camino-fisterra.json camino-frances.json /app/lib
...
EXPOSE 3000
CMD /app/bin/camino-server-exe -s /app/lib/static -c /app/lib/config-static.yaml /app/lib/camino-portuguese.json /app/lib/camino-fisterra.json /app/lib/camino-frances.json
