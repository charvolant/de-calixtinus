...
The track will be far too detailed for de-calixtinus to use and will generally be a bit noisy.
...
You will need an XSL processor.
On linux systems, this will be `xsltproc` and use the command
`xsltproc -o track.csv gpx.xslt track.gpx` replacing *track* with the
appropriate file name.

Mac users can install `xsltproc`
via [macports](https://ports.macports.org/port/libxslt/)
or [brew](https://formulae.brew.sh/formula/libxslt).
...
You can then use the tracks to correctly place locations.
