{-# LANGUAGE OverloadedStrings #-}
{-# LANGUAGE QuasiQuotes #-}
{-# LANGUAGE TemplateHaskell #-}
{-# OPTIONS_HADDOCK prune #-}
{-|
Module      : Css
Description : Produce Css styles for HTML and KML display
Copyright   : (c) Doug Palmer, 2023
License     : MIT
Maintainer  : doug@charvolant.org
Stability   : experimental
Portability : POSIX

Generate CSS for HTML and KML output.
-}
module Camino.Display.Css (
  -- * Generate CSS
    caminoCss
  , staticCss
  -- * Generate route line styles and colours
  , featureLineStyle
  , featureRouteColour
  , featureRouteDesaturatedColour
  -- * Standard colours
  , caminoBlue
  , caminoLightGrey
  , caminoYellow
  , informationBlue
  , mutedBlue
  , recreationGreen
  , successGreen
  , warningRed
  -- * Conversion
  , toCssColour
  , toExcelColour
) where

import Camino.Camino
import Camino.Colour
import Camino.Config
import Camino.Display.Routes
import Data.Char (ord)
import Data.Default.Class
import Data.Localised (rootLocale)
import Data.Text (Text)
import Numeric
import Text.Cassius
import Text.Hamlet (Render)

paletteCss :: Text -> Palette -> Render CaminoRoute -> Css
paletteCss ident pal = [cassius|
.#{ident}
  h1
    color: #{toCssColour $ paletteTextColour pal}
  h2
    color: #{toCssColour $ paletteTextColour pal}
  h3
    color: #{toCssColour $ paletteTextColour pal}
  h4
    color: #{toCssColour $ paletteTextColour pal}
  h5
    color: #{toCssColour $ paletteTextColour pal}
  |]
  
iconCss :: String -> Char -> Render CaminoRoute -> Css
iconCss ident ch = [cassius|
.#{ident}::before
  font-family: "Camino Icons"
  font-weight: normal
  line-height: 1
  text-rendering: auto
  content: "\#{hex $ ord ch}"
|]
  where
    hex c = showHex c ""

iconList :: [(String, Char)]
iconList = [
    ("ca-accessible", '\xe067'),
    ("ca-airport", '\xe056'),
    ("ca-albergue", '\xe010'),
    ("ca-altar", '\xe030'),
    ("ca-arduous", '\xe080'),
    ("ca-artwork", '\xe050'),
    ("ca-bank", '\xe042'),
    ("ca-bath", '\xe05a'),
    ("ca-beach", '\xe01f'),
    ("ca-bed-double", '\xe022'),
    ("ca-bed-double-wc", '\xe023'),
    ("ca-bed-quadruple", '\xe026'),
    ("ca-bed-quadruple-wc", '\xe027'),
    ("ca-bed-single", '\xe020'),
    ("ca-bed-triple", '\xe024'),
    ("ca-bed-triple-wc", '\xe025'),
    ("ca-bedlinen", '\xe06c'),
    ("ca-bishop", '\xe031'),
    ("ca-breakfast", '\xe064'),
    ("ca-bicycle-repair", '\xe045'),
    ("ca-bicycle-storage", '\xe06a'),
    ("ca-bridge", '\xe004'),
    ("ca-bus", '\xe047'),
    ("ca-bus-link", '\xe084'),
    ("ca-calendar", '\xe095'),
    ("ca-campground", '\xe019'),
    ("ca-camping", '\xe019'),
    ("ca-casa-rural", '\xe012'),
    ("ca-cathedral", '\xe008'),
    ("ca-church", '\xe014'),
    ("ca-city", '\xe003'),
    ("ca-clock", '\xe094'),
    ("ca-cooling", '\xe071'),
    ("ca-copy", '\xe09d'),
    ("ca-cross", '\xe009'),
    ("ca-cycling", '\xe081'),
    ("ca-detail", '\xe09e'),
    ("ca-dinner", '\xe065'),
    ("ca-document", '\xe099'),
    ("ca-document-kml", '\xe09b'),
    ("ca-document-spreadsheet", '\xe09a'),
    ("ca-dryer", '\xe062'),
    ("ca-farmland", '\xe057'),
    ("ca-ferry", '\xe082'),
    ("ca-festival", '\xe032'),
    ("ca-food", '\xe036'),
    ("ca-fountain", '\xe00a'),
    ("ca-globe", '\xe090'),
    ("ca-gite", '\xe01a'),
    ("ca-groceries", '\xe041'),
    ("ca-guesthouse", '\xe012'),
    ("ca-halfboard", '\xe074'),
    ("ca-handwash", '\xe063'),
    ("ca-hard", '\xe080'),
    ("ca-hazard", '\xe093'),
    ("ca-heating", '\xe070'),
    ("ca-help", '\xe092'),
    ("ca-historical", '\xe00d'),
    ("ca-holiday", '\xe034'),
    ("ca-homestay", '\xe011'),
    ("ca-hostel", '\xe010'),
    ("ca-hotel", '\xe013'),
    ("ca-house", '\xe011'),
    ("ca-house-small", '\xe098'),
    ("ca-industry", '\xe058'),
    ("ca-information", '\xe091'),
    ("ca-information-point", '\xe053'),
    ("ca-intersection", '\xe005'),
    ("ca-junction", '\xe01e'),
    ("ca-key", '\xe09f'),
    ("ca-kitchen", '\xe06b'),
    ("ca-label", '\xe09c'),
    ("ca-link", '\xe096'),
    ("ca-lockers", '\xe066'),
    ("ca-lookout", '\xe051'),
    ("ca-map", '\xe097'),
    ("ca-mattress", '\xe028'),
    ("ca-medical", '\xe044'),
    ("ca-monastery", '\xe006'),
    ("ca-municipal", '\xe00b'),
    ("ca-museum", '\xe00c'),
    ("ca-music", '\xe033'),
    ("ca-natural", '\xe00f'),
    ("ca-park", '\xe00e'),
    ("ca-peak", '\xe007'),
    ("ca-pets", '\xe069'),
    ("ca-pharmacy", '\xe043'),
    ("ca-pilgrim", '\xe072'),
    ("ca-poi", '\xe000'),
    ("ca-pool", '\xe06e'),
    ("ca-prayer", '\xe06f'),
    ("ca-promontory", '\xe052'),
    ("ca-refuge", '\xe01a'),
    ("ca-restpoint", '\xe073'),
    ("ca-restaurant", '\xe040'),
    ("ca-rowing", '\xe083'),
    ("ca-shared", '\xe021'),
    ("ca-shop", '\xe01d'),
    ("ca-shrine", '\xe05b'),
    ("ca-sleeping-bag", '\xe028'),
    ("ca-stables", '\xe068'),
    ("ca-station", '\xe055'),
    ("ca-statue", '\xe01b'),
    ("ca-stockpoint", '\xe041'),
    ("ca-stop", '\xe020'),
    ("ca-tent", '\xe018'),
    ("ca-theatre", '\xe035'),
    ("ca-towels", '\xe06d'),
    ("ca-town", '\xe002'),
    ("ca-train", '\xe046'),
    ("ca-train-link", '\xe085'),
    ("ca-village", '\xe001'),
    ("ca-walking", '\xe080'),
    ("ca-warning", '\xe093'),
    ("ca-washing-machine", '\xe061'),
    ("ca-waterbody", '\xe059'),
    ("ca-wharf", '\xe054'),
    ("ca-wifi", '\xe060'),
    ("ca-winery", '\xe01c')
  ]
  
caminoIconCss :: [Render CaminoRoute -> Css]
caminoIconCss = map (\(ident, ch) -> iconCss ident ch) iconList

caminoFontCss :: AssetConfig -> Render CaminoRoute -> Css
caminoFontCss asset = [cassius|
@font-face
  font-family: "#{ident}"
  font-weight: normal
  font-style: normal
  src: url(@{AssetRoute ident})
|]
  where
    ident = assetId asset

caminoBaseCss :: Render CaminoRoute -> Css
caminoBaseCss = $(cassiusFile "templates/css/base.cassius")

-- | Generate CSS that can be placed in a static file
--
-- Use this function to generate CSS that can be placed in a static file, served from a common point.
-- Fonts and graphics are linked to the URLs provided in the configuration.
staticCss :: Config -- ^ The base configuration
  -> [Render CaminoRoute -> Css] -- ^ A list of CSS fragments for static CSS
staticCss config = [caminoBaseCss, paletteCss "location-default" def] ++ (map caminoFontCss (getAssets Font config)) ++ caminoIconCss


-- | The colour used to display a used route
featureRouteColour :: Route -> PaletteColour
featureRouteColour route = paletteColour $ routePalette route

-- | A desaturated version of the `featureRouteColour` suitable for use displaying an unused route
featureRouteDesaturatedColour :: Route -> PaletteColour
featureRouteDesaturatedColour route = lighten c
  where c = paletteColour $ routePalette route

featureRouteBlueColour :: Route -> PaletteColour
featureRouteBlueColour _route = ferryGreen

featureRouteGreyColour :: Route -> PaletteColour
featureRouteGreyColour _route = unusedGrey


-- | Generate information about a feature's type
--
--   This can be used as a start point for generating SVG line styles, colours and the like.
--   It can also be used to specify [Leaflet](https://leafletjs.com) line styles.
--   Generally:
--
--   * Used routes and features are thicker than unused routes an features
--   * Non-road features have combinations of dashes to indicate
--   * Transport routes, such as ferries tend to have unique colours
featureLineStyle :: Route -- ^ The associated route 
  -> Bool -- ^ Is this a large-scale line (showing rough point-to-point)
  -> Bool -- ^ Is this a dummy feature (one intended as a placholder for a more sophisticated entry later)
  -> Bool -- ^ Is this a used feature (one which the pilgrim will travel along)
  -> LegType -- ^ The type of feature 
  -> (PaletteColour, Int, Float, Float, Maybe [Int], Maybe String) -- ^ (Feature RGBhex colour, line width, line opacity, line opacity when dashes are not available, optional dah array, optional cap type)
featureLineStyle route True _ True _ = (featureRouteColour route, 6, 1.0, 1.0, Nothing, Nothing)
featureLineStyle route True _ False _ = (featureRouteDesaturatedColour route, 4, 1.0, 1.0, Nothing, Nothing)
featureLineStyle route False True True _ = (featureRouteColour route, 6, 1.0, 0.5, Just [6, 6], Just "round")
featureLineStyle route False True False _ = (featureRouteColour route, 4, 0.5, 0.5, Just [6, 10, 2, 10], Just "round")
featureLineStyle route False False True Trail = (featureRouteColour route, 6, 1.0, 1.0, Just [6, 6], Just "round")
featureLineStyle route False False True CyclePath = (featureRouteColour route, 6, 1.0, 1.0, Just [20, 6, 4, 6], Just "round")
featureLineStyle route False False True FerryLink = (featureRouteBlueColour route, 6, 1.0, 1.0, Nothing, Nothing)
featureLineStyle route False False True BoatLink = (featureRouteBlueColour route, 6, 1.0, 1.0, Nothing, Nothing)
featureLineStyle route False False True TrainLink = (featureRouteGreyColour route, 6, 1.0, 1.0, Just [4, 4, 2, 4], Just "butt")
featureLineStyle route False False True _ = (featureRouteColour route, 6, 1.0, 1.0, Nothing, Nothing)
featureLineStyle route False False False Trail = (featureRouteColour route, 4, 1.0, 0.5, Just [6, 10], Just "round")
featureLineStyle route False False False CyclePath = (featureRouteColour route, 4, 1.0, 0.5, Just [16, 10, 2, 10], Just "round")
featureLineStyle route False False False FerryLink = (featureRouteBlueColour route, 4, 1.0, 0.5,Just [6, 10], Just "round")
featureLineStyle route False False False BoatLink = (featureRouteBlueColour route, 4, 1.0, 0.5, Just [6, 10], Just "round")
featureLineStyle route False False False TrainLink = (featureRouteGreyColour route, 4, 1.0, 0.5, Just [4, 6, 2, 6], Just "butt")
featureLineStyle route False _ _ _ = (featureRouteColour route, 4, 1.0, 0.5, Just [6, 10], Just "round")

-- | Generate CSS for a camino.
--
--   This is intended to be embedded in a camino plan and contains things like specific route palettes.
--   It is used in conjunction with the static CSS generated by `statiCss`.
caminoCss :: Config -> Camino -> [Css]
caminoCss config camino = map (\c -> c router) (default':routes')
  where
    router = renderCaminoRoute config [rootLocale]
    default' = paletteCss "location-default" (routePalette $ caminoDefaultRoute camino)
    routes' =  map (\r -> paletteCss ("location-" <> routeID r) (routePalette r)) (caminoRoutes camino)
