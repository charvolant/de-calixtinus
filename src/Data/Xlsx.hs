...
data CellID =
  ...
  | AbsoluteCellID CellID -- ^ Absolute versionb of a cell identifier
  | OffsetCellID Int Int CellID -- ^ Offset version of a cell identifier
...
toAbsolute cid = AbsoluteCellID cid

toOffset :: Int -> Int -> CellID -> CellID
toOffset ro co cid = OffsetCellID ro co cid
...
data Formula =
  ...
  | FRange CellID CellID -- ^ A relative reference to a range of cells
  | FColumn Int Formula -- ^ A column within another formula
  | FApply Text [Formula] -- ^ Function application
  deriving (Show)

-- | Offset the cell positions in a formula.
--   A utility function to allow shifting a base formula across a sheet
offsetFormula :: Int -> Int -> Formula -> Formula
offsetFormula ro co (FRef cid) = FRef (OffsetCellID ro co cid)
offsetFormula ro co (FRange cid1 cid2) = FRange (OffsetCellID ro co cid1) (OffsetCellID ro co cid2)
offsetFormula ro co (FApply name args) = FApply name (map (offsetFormula ro co) args)
offsetFormula _ _ f = f
...
cellIDLookup' :: M.Map CellID (Text, RowCoord, ColumnCoord) -> CellID -> (Text, RowCoord, ColumnCoord)
cellIDLookup' cmap cid@(CellID _ _) = maybe (error ("Can't map cell ID " ++ show cid)) id (M.lookup cid cmap)
cellIDLookup' cmap (AbsoluteCellID cid) = (title, toRowAbsolute rc, toColumnAbsolute cc) where
  (title, rc, cc) = cellIDLookup' cmap cid
cellIDLookup' cmap (OffsetCellID ro co cid) = (title, addRowCoord rc ro, addColumnCoord cc co) where
  (title, rc, cc) = cellIDLookup' cmap cid

cellIDLookup :: M.Map CellID (Text, RowCoord, ColumnCoord) -> Text -> CellID -> CellRef
cellIDLookup cmap title cid = let
  (title', rc, cc) = cellIDLookup' cmap cid
  in
    if Data.Text.null title' || title == title' then
      singleCellRef' (rc, cc)
    else
      mkForeignSingleCellRef title' (rc, cc)

cellRange :: (CellID -> CellRef) -> CellID -> CellID -> Range
cellRange cells cid1 cid2 = let
    cref1 = cells cid1
    cref2 = cells cid2
    sheet = maybe Nothing (Just . fst) (fromForeignSingleCellRef cref1)
    coords1 = maybe (error ("Can't decode reference " ++ show cref1 ++ " from " ++ show cid1)) id $ fromSingleCellRef' cref1
    coords2 = maybe (error ("Can't decode reference " ++ show cref2 ++ " from " ++ show cid2)) id $ fromSingleCellRef' cref2
  in
    maybe (mkRange' coords1 coords2) (\s -> mkForeignRange s coords1 coords2) sheet

cellColumn :: (CellID -> CellRef) -> Int -> Formula -> Range
cellColumn cells offset (FRef cid) = cellRange cells (OffsetCellID 0 offset cid) (OffsetCellID 0 offset cid)
cellColumn cells offset (FRange cid1 cid2) = let
    cref1 = cells cid1
    cref2 = cells cid2
    sheet = maybe Nothing (Just . fst) (fromForeignSingleCellRef cref1)
    (r1, c1) = fromJust $ fromSingleCellRef' cref1
    (r2, _c2) = fromJust $ fromSingleCellRef' cref2
    coords1 = (r1, addColumnCoord c1 offset)
    coords2 = (r2, addColumnCoord c1 offset)
  in
    maybe (mkRange' coords1 coords2) (\s -> mkForeignRange s coords1 coords2) sheet
cellColumn _ _ f = error ("Can't turn " ++ show f ++ " into a column")

createCellMap''' :: Text -> Int -> Int -> Cell t -> [(CellID, (Text, RowCoord, ColumnCoord))] -> [(CellID, (Text, RowCoord, ColumnCoord))]
createCellMap''' title row column cell cm = maybe
  cm
  (\cid -> let
      (r, c) = absolute row column cell
    in
      (cid, (title, RowRel r, ColumnRel c)):cm
    )
  (cell ^. cellID)

createCellMap'' :: Int -> Int -> Text -> Slab (Cell t) -> ([(CellID, (Text, RowCoord, ColumnCoord))], Text)
...
createCellMap' :: Renderer t -> Worksheet t -> M.Map CellID (Text, RowCoord, ColumnCoord)
...
formulaFromFormula' cells _prec (FRange ref1 ref2) = (unCellRef $ cellRange cells ref1 ref2)
formulaFromFormula' cells _prec (FColumn offset v) = (unCellRef $ cellColumn cells offset v)
